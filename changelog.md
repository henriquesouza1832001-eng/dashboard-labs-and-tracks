...
## Por que salvar em Obras demorava 17-50 segundos

Isso foi a investigação mais longa até agora. Salvar qualquer coisa em Obras — mesmo só 1 campo de budget — demorava um absurdo, e se duas pessoas tentassem salvar coisas diferentes ao mesmo tempo, uma delas quebrava.

Fui descascando isso em camadas, e cada vez que eu achava a causa e corrigia, o problema voltava com outra cara:
...
Enquanto tava nisso, percebi que nenhuma escrita tinha retry para quando o Delta Lake reclama de escrita concorrente (DELTA_CONCURRENT_APPEND — acontece quando duas escritas batem na mesma tabela ao mesmo tempo). Sem isso, a segunda pessoa que tentasse salvar simplesmente recebia erro. Adicionei uma função de retry com backoff em todas as rotas de escrita do sistema.
...
**Resultado:** de 17-50 segundos pra ~2 segundos, no mesmo teste.

**O que eu aprendi com isso:** quando algo tá lento, checar na ordem — o backend tá fazendo muita chamada? tá lendo mais do banco do que precisa? e só depois, o frontend tá mandando mais dado do que devia? Essa última foi a que demorei mais pra enxergar, porque os sintomas (tempo alto, resposta pequena) apontavam só pro servidor.

---

## Bug de segurança achado no meio do caminho

Durante essa mesma revisão, achei que a rota de registrar avanço físico de uma etapa (registrar_avanco) montava parte do SQL colando os valores direto na string, em vez de usar parâmetro — isso é SQL injection clássico, mesmo sem intenção de ter feito assim. Corrigido pra usar bind de parâmetro em tudo. Depois disso, revisei o resto do arquivo procurando o mesmo padrão e não achei outro caso igual.
...
- Uma coluna que não existia mais na tabela de manutenções (foi normalizada pra outra tabela, `manutencao_pecas`) ainda tava sendo referenciada — corrigido.
...
- Todas as 10 funções de salvar do Conforto foram reestruturadas pra lote — isso sozinho resolveu um timeout que acontecia com muitas UCs/manutenções de uma vez.
- Achei um bug de digitação na rota de criar requisição via QR: a chave usada pra buscar o id da peça no corpo da requisição tava escrita errada, então toda peça salva por ali ficava sem vínculo nenhum, sem erro nenhum aparecer — só ficava silenciosamente quebrado. Corrigido.

---

## Curva S por item de checklist, em Obras

Implementei o cálculo de avanço no estilo MS Project — peso por duração de cada item/sub-tarefa/etapa, em vez de contagem simples de "quantos concluídos".
