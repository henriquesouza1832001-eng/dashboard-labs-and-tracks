...
if(dCh){
    ...
    document.getElementById('mkpis-chamados').innerHTML=
      `<div class="mod-micro-kpi" onclick="abrirModuloComDrill('chamados','ch-total')" style="cursor:pointer" title="Abrir Total chamados"><div class="mod-micro-lbl">Total chamados</div><div class="mod-micro-val c-azul">${total}</div></div>`+
...
function abrirModuloComDrill(modulo,tipo){
  if(moduloAtivo!==modulo){
    ...
    if(modulo==='obras'){
      ...
      window._kpiDados_capex = d;
      try{ sessionStorage.setItem('_kpi_capex', JSON.stringify(d)); }catch(_){}
      abrirDrillCapex(tipo);
    }
  ...
  setTimeout(()=>toggleObCardGenerico('ergonomia',tipo),200);
    ...
    else if(modulo==='capex') abrirDrillCapex(tipo);
...
function desenharCurvaS(canvasId, obra, lancs, budgetTotal, modo='fisico') {
  ...
  if (!etapas.length && !lancs.length) {
    ...
    const toMs2 = d => new Date(d).getTime();
    const ini = toMs2(dtIni), fim = toMs2(dtFim), hoje = Date.now();
    const meses2 = [];
    let cur2 = new Date(ini); cur2.setDate(1);
    const fim2 = new Date(fim); fim2.setDate(1);
    while(cur2 <= fim2) { meses2.push(cur2.toISOString().slice(0,7)); cur2 = new Date(cur2.getFullYear(), cur2.getMonth()+1, 1); }
    if(!meses2.length) return;
    const xPos2 = i => m.l + (i / (meses2.length-1||1)) * cw;
    const yPos2 = v => m.t + ch - (v / 100) * ch;
    ctx.lineWidth = 1;
    ...
    const passo2 = Math.max(1, Math.floor(meses2.length/6));
    meses2.forEach((mes,i) => {
      if(i % passo2 !== 0 && i !== meses2.length-1) return;
      ctx.fillStyle = 'rgba(139,148,158,0.7)';
...
let _capexCotacoes = { USD:6.0, EUR:6.5, ARS:0.006, BRL:1 };
(async ()=>{
  try{
    const r = await fetch('https://open.er-api.com/v6/latest/BRL');
    const j = await r.json();
    if(j.rates){ _capexCotacoes.USD=1/(j.rates.USD||1); _capexCotacoes.EUR=1/(j.rates.EUR||1); _capexCotacoes.ARS=1/(j.rates.ARS||1); }
  }catch(_){}
})();
function _cxToBRL(val, moeda){ return (val||0) * (_capexCotacoes[moeda]||1); }
function _cxFmtValor(val, moeda){
  if(!moeda||moeda==='BRL') return fmtRKs(val);
  const simb = {USD:'$',EUR:'€',ARS:'$AR'}[moeda]||moeda;
  const brl  = _cxToBRL(val, moeda);
  const fmt  = v=>v>=1e6?(v/1e6).toFixed(1)+'M':v>=1e3?(v/1e3).toFixed(0)+'k':Math.round(v).toString();
  return `${simb}${fmt(val)} <span style="font-size:10px;color:var(--text-muted);font-weight:400">= R$${fmt(brl)}</span>`;
}
function abrirDrillCapex(tipo){
  ...
  const titulos = {'cx-total':'Todos os Projetos','cx-aprov':'Aprovados','cx-analise':'Em Análise / Rascunho','cx-docs':'Sem Documentação'};
  ...
  overlay.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:18px">
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px;display:flex;flex-direction:column;align-items:center;gap:6px">
        ...
        <div style="font-size:10px;color:var(--text-muted)">R$${(aprTotal/1e3).toFixed(0)}k aprovado de R$${(solTotal/1e3).toFixed(0)}k</div>
      </div>
      <!-- 2. Barras por planta -->
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px;display:flex;flex-direction:column;align-items:center">
        ...
        <div style="margin-top:8px;font-size:10px;display:flex;flex-wrap:wrap;gap:4px;justify-content:center">
          ${Object.entries(statusMap).map(([s,n])=>`<span style="background:${corStatus[s]||'#8a9abf'}22;color:${corStatus[s]||'#8a9abf'};padding:1px 6px;border-radius:3px;font-family:var(--mono)">${s}: ${n}</span>`).join('')}
        </div>
      </div>
    </div>
    <!-- LISTA DE PROJETOS -->
    <div style="font-size:10px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px">Projetos</div>
    ...
    lista.map(p=>{
      ...
      const solBRL = _cxToBRL(sol, p.moeda);
      const aprBRL = _cxToBRL(apr, p.moeda);
      const pct = solBRL>0?Math.min(aprBRL/solBRL,1):0;
      const cor = corStatus[p.status]||'#8a9abf';
      const temArq = (p.arquivos||[]).length>0;
      const itens = p.itens||[];
      const moedaDif = p.moeda && p.moeda!=='BRL';
      return`<div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px 16px;margin-bottom:10px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:10px">
          <div>
            <div style="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Solicitado</div>
            <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--blue-mid)">${_cxFmtValor(sol,p.moeda)}</div>
          </div>
          ${apr>0?`<div>
            <div style="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Aprovado</div>
            <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:#1a7f4b">${_cxFmtValor(apr,p.moeda)}</div>
          </div>`:''}
          <div>
            <div style="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Documentos</div>
            <div style="font-size:12px;font-weight:600;color:${temArq?'#1a7f4b':'#c0392b'}">${temArq?'✓ Completo':'✗ Pendente'}</div>
          </div>
          ${itens.length?`<div>
            <div style="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">Itens de custo</div>
            <div style="font-size:12px;font-weight:600;color:var(--text)">${itens.length} item${itens.length>1?'s':''}</div>
          </div>`:''}
        </div>
        ${solBRL>0?`<div style="height:5px;background:#e8edf5;border-radius:3px;overflow:hidden">
          <div style="height:100%;width:${pct*100}%;background:#1a7f4b;border-radius:3px;transition:width .4s"></div>
        </div>`:''}
        ${p.justificativa?`<div style="font-size:11px;color:var(--text-muted);margin-top:8px;font-style:italic;border-left:2px solid var(--border);padding-left:8px">${p.justificativa}</div>`:''}
      </div>`;
    }).join('')}`;
  conteudo.appendChild(overlay);
  setTimeout(()=>{
    ...
    const bCanvas = document.getElementById('cx-barras');
    if(bCanvas && porPlanta.length){
      const fmtK = v=>v>=1e6?'R$'+(v/1e6).toFixed(1)+'M':v>=1e3?'R$'+(v/1e3).toFixed(0)+'k':'R$'+Math.round(v);
      desenharBarrasH('cx-barras', porPlanta.map(x=>x.nome), porPlanta.map(x=>x.sol), ['#3a6bc7']);
    }
    const sl = Object.entries(statusMap);
    if(sl.length) desenharDonutNaCanvas('cx-donut', sl.map(x=>x[0]), sl.map(x=>x[1]), sl.map(x=>corStatus[x[0]]||'#8a9abf'));
  },80);
}
