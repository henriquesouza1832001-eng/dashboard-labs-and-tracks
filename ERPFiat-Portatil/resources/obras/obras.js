...
function avancFisico(obraCod){
  ...
  const exec = o.etapas.reduce((s,e)=>s+(e.peso||1)*afEtapa(e)/100,0);
        ...
        <td>
          ...
          <div style="display:flex;align-items:center;gap:6px">
            <div style="flex:1;height:6px;background:var(--border);border-radius:3px">
              <div style="width:${afEtapa(e)}%;height:100%;background:var(--blue-mid);border-radius:3px"></div>
            </div>
            <span style="font-family:var(--mono);font-size:11px;min-width:36px">${fmt(afEtapa(e),0)}%</span>
          </div>` : `<span style="font-size:11px;color:var(--text-muted)">0% — adicione uma sub-tarefa</span>`}
        </td>
        <td>
...
function renderBudget(){
  ...
  tbody.innerHTML=state.budget.map((b,idx)=>{ const real=state.lancamentos.filter(l=>l.obraCod===b.obraCod&&l.cresp===b.cresp).reduce((s,l)=>s+l.qtd*l.precoUnit,0); const totalDisp=(b.budgetAprov||0)+(b.contingencia||0),aFaturar=totalDisp-real,pct=totalDisp>0?(real/totalDisp)*100:0; tB+=b.budgetAprov||0;tC+=b.capex||0;tO+=b.opex||0;tCont+=b.contingencia||0;tR+=real; return `<tr><td><span class="badge badge-muted">${b.obraCod}</span></td><td><span class="badge badge-blue">${b.cresp}</span></td><td><span class="badge ${b.tipoVerba==='CAPEX'?'badge-yellow':'badge-blue'}">${b.tipoVerba}</span></td><td style="font-family:var(--mono);font-size:12px">${fmtR(b.budgetAprov)}</td><td style="font-family:var(--mono);font-size:12px">${fmtR(b.capex)}</td><td style="font-family:var(--mono);font-size:12px">${fmtR(b.opex)}</td><td style="font-family:var(--mono);font-size:12px">${fmtR(b.contingencia)}</td><td style="font-family:var(--mono);font-size:12px">${fmtR(totalDisp)}</td><td style="font-family:var(--mono);font-size:12px">${fmtR(real)}</td><td>${progressBar(pct)}</td><td style="font-family:var(--mono);font-size:12px;color:${aFaturar<0?'var(--red)':'var(--green)'}">${fmtR(aFaturar)}</td><td><span class="badge ${badgeBudget(pct)}">${pct>=100?'ESTOURADO':pct>=85?'ATENÇÃO':'OK'}</span></td><td style="position:sticky;right:0;background:#0d1117;z-index:1"><div class="row-actions"><button class="action-btn" onclick="editarBudget(${idx})"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button class="action-btn danger" onclick="excluirBudget(${idx})"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button></div></td></tr>`; }).join('');
...
function editarObra(idx){ const o=state.obras[idx]; state.editIdx.obra=idx; $('modal-obra-title').textContent='Editar Obra'; $('ob-cod').value=o.cod; $('ob-status').value=o.status; $('ob-nome').value=o.nome; $('ob-tipo').value=o.tipo||''; $('ob-local').value=o.local||''; $('ob-resp').value=o.responsavel||''; $('ob-cresp').value=o.cresp||''; $('ob-dt-ini-prev').value=o.dtInicioPrev||''; $('ob-dt-fim-prev').value=o.dtFimPrev||''; $('ob-dt-ini-real').value=o.dtInicioReal||''; $('ob-dt-fim-real').value=o.dtFimReal||''; $('ob-crono-ini').value=o.cronogramaIniData||''; $('ob-crono-fim').value=o.cronogramaFimData||''; $('ob-obs').value=o.obs||''; $('ob-err').textContent=''; abrirModal('modal-obra'); }
