...
O banco ser Delta Lake em vez de, por exemplo, um Postgres comum, veio de aproveitar a infraestrutura que já existia no Databricks (é o ambiente onde os dados da empresa já vivem). A troca por essa conveniência foi um conjunto de restrições que moldou bastante decisão de código — tem uma seção só sobre isso mais abaixo.
...
Três variações de tela em cima do mesmo módulo de Chamados, pensadas para públicos diferentes: "Meus Chamados" é a visão de quem abriu o chamado, "Service Desk" é o painel de quem tria e distribui, e "Operador" é uma tela mais direta para o time de facilities executar. Não passaram pela mesma revisão de performance/cache que os módulos principais — funcionam sobre as mesmas rotas de `/api/chamados`, então herdam o comportamento de lá, mas as telas em si nunca foram abertas linha por linha numa auditoria.
...
- Um cookie `ctrl-token`, `httponly` e `secure`, válido por 12 horas — protege as páginas HTML no servidor (tentar abrir `/obras` sem esse cookie válido redireciona direto para `/login`).
...
| `avanco_fisico` | DOUBLE | 0-100, soma ponderada dos itens concluídos nas sub-tarefas |
| `orcamento` | DOUBLE | |
...
Esse é o fluxo que mais mudou ao longo do projeto, então vale contar como ele funciona hoje versus como funcionava antes.

Você abre o detalhe de uma obra, vai na aba Budget/CAPEX, preenche o modal e clica em salvar. No frontend, isso adiciona (ou substitui) um objeto dentro de `state.budget` — o array que guarda todo o budget do sistema em memória, no navegador. Junto com isso, o id desse item é marcado como "sujo" num `Set` que rastreia o que mudou desde o último salvamento (`marcarSujo('budget', obj.id)`).
...
Quando `salvarDados()` roda, ele filtra `state.obras`, `state.budget` e `state.lancamentos` para conter *apenas* os itens marcados como sujos, e manda isso — não o estado inteiro — para `POST /api/obras`.
...
Só então a resposta `{"ok": true}` volta pro frontend, que limpa o `Set` de itens sujos e atualiza o indicador visual de "salvo".
